namespace Tests.Workspace
{
    ...
    public static class AssertExtensions
    {
        ...
        public static void ShouldEqual(this object actual, object expected, Context context)
          => Assert.True(Equals(actual, expected), $"Expected Equals: [{actual}, {expected}] on context {context}");
       ...
       => Assert.True(!(actual is null), $"Expected Not Null: [{actual}] on context {context} with mode1 {mode}");
        ...
        public static void ShouldNotBeNull(this object actual, Context context)
            => Assert.True(!(actual is null), $"Expected Not Null: [{actual}] on context {context}");
        ...
        public static void ShouldContains(this IEnumerable<IObject> collection, IObject expected, Context context)
        => Assert.True(collection.Contains(expected), $"Expected Not Null: [{collection}, {expected}] on context {context}");

        #endregion

        #region ShouldNotContains
        public static void ShouldNotContains(this IEnumerable<IObject> collection, IObject expected, Context context)
            => Assert.True(!collection.Contains(expected), $"Expected Not Contains: [{collection}, {expected}] on context {context}");
