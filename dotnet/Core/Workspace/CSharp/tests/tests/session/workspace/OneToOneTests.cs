            ...
            foreach (var push in this.pushes)
            {
                foreach (WorkspaceMode mode in Enum.GetValues(typeof(WorkspaceMode)))
                {
                    foreach (var contextFactory in this.contextFactories)
                    {
                        ...
                        c1x_1.SessionC1WorkspaceC1One2One = c1y_1;

                        c1x_1.SessionC1WorkspaceC1One2One.ShouldEqual(c1y_1, ctx, mode);
                        c1y_1.SessionC1WhereSessionC1WorkspaceC1One2One.ShouldEqual(c1x_1, ctx, mode);
            ...
            foreach (var push in this.pushes)
            {
                foreach (WorkspaceMode mode in Enum.GetValues(typeof(WorkspaceMode)))
                {
                    foreach (var contextFactory in this.contextFactories)
                    {
                        ...
                        c1x_1.RemoveSessionC1WorkspaceC1One2One();
                        c1x_1.SessionC1WorkspaceC1One2One.ShouldNotEqual(c1y_1, ctx, mode);
                        c1y_1.SessionC1WhereSessionC1WorkspaceC1One2One.ShouldNotEqual(c1x_1, ctx, mode);
