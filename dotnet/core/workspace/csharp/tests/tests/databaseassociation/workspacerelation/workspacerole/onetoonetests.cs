                    ...
                    foreach (DatabaseMode mode1 in Enum.GetValues(typeof(DatabaseMode)))
                    {
                        foreach (WorkspaceMode mode2 in Enum.GetValues(typeof(WorkspaceMode)))
                        {
                            foreach (var contextFactory in this.contextFactories)
                            {
                                ...
                                var c1x = await ctx.Create<C1>(session1, mode1);
                                ...
                                c1x.WorkspaceWC1One2One = c1y_1;

                                c1x.WorkspaceWC1One2One.ShouldEqual(c1y_1, ctx, mode1, mode2);
                                c1y_1.C1WhereWorkspaceWC1One2One.ShouldEqual(c1x, ctx, mode1, mode2);
                    ...
                    foreach (DatabaseMode mode1 in Enum.GetValues(typeof(DatabaseMode)))
                    {
                        foreach (WorkspaceMode mode2 in Enum.GetValues(typeof(WorkspaceMode)))
                        {
                            foreach (var contextFactory in this.contextFactories)
                            {
                                ...
                                c1x.RemoveWorkspaceWC1One2One();
                                c1x.WorkspaceWC1One2One.ShouldNotEqual(c1y_1, ctx, mode1, mode2);
                                c1y_1.C1WhereWorkspaceWC1One2One.ShouldNotEqual(c1x, ctx, mode1, mode2);
