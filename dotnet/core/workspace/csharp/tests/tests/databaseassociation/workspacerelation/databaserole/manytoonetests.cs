                ...
                foreach (DatabaseMode mode1 in Enum.GetValues(typeof(DatabaseMode)))
                {
                    foreach (DatabaseMode mode2 in Enum.GetValues(typeof(DatabaseMode)))
                    {
                        foreach (var contextFactory in this.contextFactories)
                        {
                            ...
                            c1x_1.WorkspaceC1Many2One = c1y_1;

                            c1x_1.WorkspaceC1Many2One.ShouldEqual(c1y_1, ctx, mode1, mode2);
                ...
                foreach (DatabaseMode mode1 in Enum.GetValues(typeof(DatabaseMode)))
                {
                    foreach (DatabaseMode mode2 in Enum.GetValues(typeof(DatabaseMode)))
                    {
                        foreach (var contextFactory in this.contextFactories)
                        {
                            ...
                            c1x_1.RemoveWorkspaceC1Many2One();
                            c1x_1.WorkspaceC1Many2One.ShouldNotEqual(c1y_1, ctx, mode1, mode2);
                            c1y_1.C1sWhereWorkspaceC1Many2One.ShouldNotContain(c1x_1, ctx, mode1, mode2);
