                    ...
                    foreach (DatabaseMode mode1 in Enum.GetValues(typeof(DatabaseMode)))
                    {
                        foreach (WorkspaceMode mode2 in Enum.GetValues(typeof(WorkspaceMode)))
                        {
                            foreach (var contextFactory in this.contextFactories)
                            {
                                ...
                                c1x_1.AddSessionWC1Many2Many(c1y_1);
                                ...
                                Assert.Single(c1y_1.C1sWhereSessionWC1Many2Many.Where(v => v.Equals(c1x_1)));
                    ...
                    foreach (var workspacePush in this.workspacePushes)
                    {
                        foreach (DatabaseMode mode1 in Enum.GetValues(typeof(DatabaseMode)))
                        {
                            foreach (WorkspaceMode mode2 in Enum.GetValues(typeof(WorkspaceMode)))
                            {
                                ...
                                c1x_1.AddSessionWC1Many2Many(null);
                                Assert.Empty(c1x_1.SessionWC1Many2Manies);
                    ...
                    foreach (DatabaseMode mode1 in Enum.GetValues(typeof(DatabaseMode)))
                    {
                        foreach (WorkspaceMode mode2 in Enum.GetValues(typeof(WorkspaceMode)))
                        {
                            foreach (var contextFactory in this.contextFactories)
                            {
                                ...
                                c1x_1.RemoveSessionWC1Many2Many(c1y_1);
                                c1x_1.SessionWC1Many2Manies.ShouldNotContain(c1y_1, ctx, mode1, mode2);
                                c1y_1.C1sWhereSessionWC1Many2Many.ShouldNotContain(c1x_1, ctx, mode1, mode2);
                    ...
                    foreach (DatabaseMode mode1 in Enum.GetValues(typeof(DatabaseMode)))
                    {
                        foreach (WorkspaceMode mode2 in Enum.GetValues(typeof(WorkspaceMode)))
                        {
                            foreach (var contextFactory in this.contextFactories)
                            {
                                ...
                                c1x_1.RemoveSessionWC1Many2Many(null);
