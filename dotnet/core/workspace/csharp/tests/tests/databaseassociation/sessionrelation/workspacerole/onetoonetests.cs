                    ...
                    foreach (DatabaseMode mode1 in Enum.GetValues(typeof(DatabaseMode)))
                    {
                        foreach (WorkspaceMode mode2 in Enum.GetValues(typeof(WorkspaceMode)))
                        {
                            foreach (var contextFactory in this.contextFactories)
                            {
                                ...
                                c1x.SessionWC1One2One = c1y_1;

                                c1x.SessionWC1One2One.ShouldEqual(c1y_1, ctx, mode1, mode2);
                                c1y_1.C1WhereSessionWC1One2One.ShouldEqual(c1x, ctx, mode1, mode2);
                    ...
                    foreach (DatabaseMode mode1 in Enum.GetValues(typeof(DatabaseMode)))
                    {
                        foreach (WorkspaceMode mode2 in Enum.GetValues(typeof(WorkspaceMode)))
                        {
                            foreach (var contextFactory in this.contextFactories)
                            {
                                ...
                                c1x.RemoveSessionWC1One2One();
                                c1x.SessionWC1One2One.ShouldNotEqual(c1y_1, ctx, mode1, mode2);
                                c1y_1.C1WhereSessionWC1One2One.ShouldNotEqual(c1x, ctx, mode1, mode2);
