    ...
    public void Commit()
    {
        if (!this.busyCommittingOrRollingBack)
        {
            try
            {
                ...
                // Build snapshots for all new/modified strategies
                // Must be done BEFORE clearing modification tracking
                var strategiesToApplySnapshot = new List<(Strategy strategy, SlotSnapshot snapshot)>();
                ...
                foreach (var kvp in this.instantiatedStrategies)
                {
                    ...
                    if (this.newObjectIds.Contains(objectId))
                    {
                        ...
                        changes.NewSnapshots.Add(snapshot);
                    ...
                    {
                        // Not new or modified, just clear local state
                        strategy.Commit();
                    }
                }
                ...
                // Apply committed snapshots to strategies AFTER successful commit
                foreach (var (strategy, snapshot) in strategiesToApplySnapshot)
                {
                    strategy.ApplyCommittedSnapshot(snapshot);
                }
