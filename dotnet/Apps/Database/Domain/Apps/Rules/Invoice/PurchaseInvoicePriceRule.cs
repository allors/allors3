        ...
        public override void Derive(ICycle cycle, IEnumerable<IObject> matches)
        {
            ...
            foreach (var @this in matches.Cast<PurchaseInvoice>().Where(v => v.ExistPurchaseInvoiceState && (v.PurchaseInvoiceState.IsCreated || v.PurchaseInvoiceState.IsRevising)))
            {
                foreach (PurchaseInvoiceItem purchaseInvoiceItem in @this.ValidInvoiceItems)
                {
                    ...
                    purchaseInvoiceItem.VatRatePercentage = purchaseInvoiceItem.VatRate?.Rate ?? 0;
                    ...
                    purchaseInvoiceItem.IrpfRatePercentage = purchaseInvoiceItem.IrpfRate?.Rate ?? 0;
                    ...
                    {
                        purchaseInvoiceItem.UnitBasePrice = new SupplierOfferings(purchaseInvoiceItem.Strategy.Transaction).PurchasePrice(@this.BilledFrom, @this.InvoiceDate, @this.DerivedCurrency, purchaseInvoiceItem.Part);
                    }
                    ...
                    if (purchaseInvoiceItem.ExistUnitBasePrice)
                    {
                        ...
                        purchaseInvoiceItem.UnitVat = purchaseInvoiceItem.ExistVatRate ? purchaseInvoiceItem.UnitPrice * (purchaseInvoiceItem.VatRatePercentage ?? 0) / 100 : 0;
