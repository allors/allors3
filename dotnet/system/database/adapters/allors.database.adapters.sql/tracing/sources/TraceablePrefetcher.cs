...
namespace Allors.Database.Adapters.Sql
{
    ...
    internal sealed class TraceablePrefetcher : Prefetcher
    {
        ...
        {
            var @event = new Event(this.Transaction, EventKind.PrefetcherPrefetchUnitRoles) { Class = @class, Associations = associations?.ToArray(), RoleType = anyRoleType };
        ...
        {
            var @event = new Event(this.Transaction, EventKind.PrefetcherPrefetchCompositeRoleObjectTable) { Associations = associations?.ToArray(), RoleType = roleType, NestedObjectIds = nestedObjectIds?.ToArray(), Leafs = leafs?.ToArray() };
            this.sink.OnBefore(@event);
        ...
        {
            var @event = new Event(this.Transaction, EventKind.PrefetcherPrefetchCompositeRoleRelationTable) { Associations = associations?.ToArray(), RoleType = roleType, NestedObjectIds = nestedObjectIds?.ToArray(), Leafs = leafs?.ToArray() };
            this.sink.OnBefore(@event);
        ...
        {
            var @event = new Event(this.Transaction, EventKind.PrefetcherPrefetchCompositesRoleObjectTable) { Associations = associations?.ToArray(), RoleType = roleType, NestedObjectIds = nestedObjectIds?.ToArray(), Leafs = leafs?.ToArray() };
            this.sink.OnBefore(@event);
        ...
        {
            var @event = new Event(this.Transaction, EventKind.PrefetcherPrefetchCompositesRoleRelationTable) { Associations = associations?.ToArray(), RoleType = roleType, NestedObjectIds = nestedObjectIds?.ToArray(), Leafs = leafs?.ToArray() };
            this.sink.OnBefore(@event);
        ...
        {
            var @event = new Event(this.Transaction, EventKind.PrefetcherPrefetchCompositeAssociationObjectTable) { Roles = roles?.ToArray(), AssociationType = associationType, NestedObjectIds = nestedObjectIds?.ToArray(), Leafs = leafs?.ToArray() };
            this.sink.OnBefore(@event);
        ...
        {
            var @event = new Event(this.Transaction, EventKind.PrefetcherPrefetchCompositeAssociationRelationTable) { Roles = roles?.ToArray(), AssociationType = associationType, NestedObjectIds = nestedObjectIds?.ToArray(), Leafs = leafs?.ToArray() };
            this.sink.OnBefore(@event);
        ...
        {
            var @event = new Event(this.Transaction, EventKind.PrefetcherPrefetchCompositesAssociationObjectTable) { Roles = roles?.ToArray(), AssociationType = associationType, NestedObjectIds = nestedObjectIds?.ToArray(), Leafs = leafs?.ToArray() };
            this.sink.OnBefore(@event);
        ...
        {
            var @event = new Event(this.Transaction, EventKind.PrefetcherPrefetchCompositesAssociationRelationTable) { Roles = roles?.ToArray(), AssociationType = associationType, NestedObjectIds = nestedObjectIds?.ToArray(), Leafs = leafs?.ToArray() };
            this.sink.OnBefore(@event);
