...
namespace Allors.Database.Configuration.Derivations.Default
{
    ...
    internal class LegacyGraph
    {
        internal LegacyGraph(LegacyDerivation derivation)
        {
            this.Derivation = derivation;
            this.NodeByObject = new Dictionary<Object, LegacyNode>();
        }

        internal LegacyDerivation Derivation { get; }

        internal Dictionary<Object, LegacyNode> NodeByObject { get; }
        ...
        private LegacyNode GetOrAddDerivationNode(Object @object)
        {
            if (!this.NodeByObject.TryGetValue(@object, out var derivationNode))
            {
                derivationNode = new LegacyNode(@object);
