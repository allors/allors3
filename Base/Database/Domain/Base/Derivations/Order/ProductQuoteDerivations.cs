...
namespace Allors.Domain
{
    ...
    {
        public class ProductQuoteCreationDerivation : IDomainDerivation
        {
            public void Derive(ISession session, IChangeSet changeSet, IDomainValidation validation)
            {
                ...
                foreach (var productQuote in createdProductQuotes)
                {
                    ...
                    var currentPriceComponents = new PriceComponents(session).CurrentPriceComponents(productQuote.IssueDate);
                    ...
                    foreach (QuoteItem quoteItem in productQuote.ValidQuoteItems)
                    {
                        ...
                        if (isSubTotalItem)
                        {
                            ...
                            {
                                validation.AddError($"{quoteItem} {M.QuoteItem.Quantity} Quantity is Required");
                            }
                        }
                        else
                        {
                            ...
                            {
                                validation.AddError($"{quoteItem} {M.QuoteItem.UnitPrice} Price is Required");
                            }
                        }
                    }
        ...
        public static void ProductQuoteRegisterDerivations(this IDatabase @this)
        {
            @this.DomainDerivationById[new Guid("bf868c8e-1581-425b-aa52-cef61788e6cb")] = new ProductQuoteCreationDerivation();
        }
    }
}
