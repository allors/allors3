...
namespace Allors.Domain
{
    ...
    public static partial class DabaseExtensions
    {
        public class UnifiedGoodCreationDerivation : IDomainDerivation
        {
            public void Derive(ISession session, IChangeSet changeSet, IDomainValidation validation)
            {
                ...
                foreach (var unifiedGood in createdUnifiedGood)
                {
                    ...
                    if (changeSet.HasChangedRoles(unifiedGood, new RoleType[] { unifiedGood.Meta.UnitOfMeasure, unifiedGood.Meta.DefaultFacility }))
                    {
                        this.SyncDefaultInventoryItem(unifiedGood);
                    }
                    ...
                    {
                        validation.AssertExists(unifiedGood, M.Good.ProductIdentifications);
                    }
                    ...
                    if (!unifiedGood.ExistDeploymentsWhereProductOffering &&
                                    ...
                                    !unifiedGood.ExistWorkEffortGoodStandardsWhereGood &&
                ...
                {
                    if (inventoryItemTransaction.Part is UnifiedGood unifiedGood)
                    {
                        DeriveQuantityOnHand(unifiedGood);
                        DeriveQuantityCommittedOut(unifiedGood);
                        DeriveAvailableToPromise(unifiedGood);
                        DeriveQuantityExpectedIn(unifiedGood);
                    }
                }
            }

            private void SyncDefaultInventoryItem(UnifiedGood unifiedGood)
            {
        ...
        public static void UnifiedGoodRegisterDerivations(this IDatabase @this)
        {
            @this.DomainDerivationById[new Guid("e19248d1-0a6e-432f-91a5-3efc852bdf70")] = new UnifiedGoodCreationDerivation();
        }
    }
}
