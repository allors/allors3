...
namespace Allors.Domain
{
    ...
    {
        public class PurchaseInvoiceCreationDerivation : IDomainDerivation
        {
            public void Derive(ISession session, IChangeSet changeSet, IDomainValidation validation)
            {
                var createdPurchaseInvoices = changeSet.Created.Select(v=>v.GetObject()).OfType<PurchaseInvoice>();

                foreach(var purchaseInvoice in createdPurchaseInvoices)
                {
                    ...
                    if (purchaseInvoice.BilledFrom is Organisation supplier)
                    {
                        ...
                        {
                            validation.AddError($"{purchaseInvoice} {purchaseInvoice.Meta.BilledFrom} {ErrorMessages.PartyIsNotASupplier}");
                        }
                    }
        ...
        public static void PurchaseInvoiceRegisterDerivations(this IDatabase @this)
        {
            @this.DomainDerivationById[new Guid("76a70135-f46e-4398-b21e-ab1e29d56a0c")] = new PurchaseInvoiceCreationDerivation();
        }
    }
}
