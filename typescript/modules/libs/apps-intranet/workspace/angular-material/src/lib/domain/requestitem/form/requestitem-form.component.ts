...
export class RequestItemFormComponent extends AllorsFormComponent<RequestItem> {
  readonly m: M;
  ...
  public serialisedItemSelected(obj: IObject): void {
    if (obj) {
      ...
      if (onOtherRequestItem) {
        this.snackBar.open(
          `Item already requested with ${onOtherRequestItem.RequestWhereRequestItem.RequestNumber}`,
          'close'
        );
      }
  ...
  private refreshSerialisedItems(product: UnifiedProduct): void {
    ...
    this.allors.context.pull(pulls).subscribe((pullResult) => {
      ...
      if (this.object.Product !== this.previousProduct) {
        ...
        this.object.AssignedUnitPrice = null;
