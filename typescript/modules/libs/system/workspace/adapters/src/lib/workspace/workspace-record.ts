...
export class WorkspaceRecord implements IRecord {
  constructor(
    ...
    public roleByRelationType: Map<RelationType, any>
  ) {}

  static fromOriginal(
    originalRecord: WorkspaceRecord,
    changedRoleByRoleType: Map<RelationType, unknown>
  ): WorkspaceRecord {
    ...
    const roleByRelationType = new Map(
      Array.from(cls.roleTypes)
        .filter(
          (roleType) =>
            changedRoleByRoleType.has(roleType.relationType) ||
            originalRoleByRelationType.has(roleType.relationType)
        )
        .map((roleType) => {
          ...
          if (changedRoleByRoleType.has(relationType)) {
            ...
            const role = originalRecord.roleByRelationType.get(
              relationType
            ) as unknown;
            return [relationType, role];
          }
