...
import { WorkEffortListPage } from '../../pages/workefforts/work-effort-list.page';

test.describe('Work Effort List', () => {
  test('should display work effort list page with table', async ({
    authenticatedPage,
  }) => {
    const listPage = new WorkEffortListPage(authenticatedPage);
    await listPage.goto();
  ...
  test('should navigate to overview on row click', async ({
    ...
    if (rowCount > 0) {
      ...
      await expect(authenticatedPage).toHaveURL(/.*\/workefforts\/worktask\//);
    }
  });
});
