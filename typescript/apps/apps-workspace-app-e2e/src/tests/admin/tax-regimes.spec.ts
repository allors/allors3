...
import { VatRegimeListPage } from '../../pages/admin/vat-regime-list.page';

test.describe('Tax Regimes', () => {
  test('should display VAT regime list page with table', async ({
    authenticatedPage,
  }) => {
    const listPage = new VatRegimeListPage(authenticatedPage);
    await listPage.goto();
  ...
  test('should navigate to overview on row click', async ({
    ...
    if (rowCount > 0) {
      ...
      await expect(authenticatedPage).toHaveURL(/.*\/admin\/vatregime\//);
    }
  });
});
