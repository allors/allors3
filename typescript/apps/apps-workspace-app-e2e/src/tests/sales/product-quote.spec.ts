...
import { ProductQuoteListPage } from '../../pages/sales/product-quote-list.page';

test.describe('Product Quote List', () => {
  test('should display product quote list page with table', async ({
    authenticatedPage,
  }) => {
    const listPage = new ProductQuoteListPage(authenticatedPage);
    await listPage.goto();
  ...
  test('should navigate to overview on row click', async ({
    ...
    if (rowCount > 0) {
      ...
      await expect(authenticatedPage).toHaveURL(/.*\/sales\/productquote\//);
    }
  });
});
