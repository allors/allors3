...
import { ProposalListPage } from '../../pages/sales/proposal-list.page';

test.describe('Proposal List', () => {
  test('should display proposal list page with table', async ({
    authenticatedPage,
  }) => {
    const listPage = new ProposalListPage(authenticatedPage);
    await listPage.goto();
  ...
  test('should navigate to overview on row click', async ({
    ...
    if (rowCount > 0) {
      ...
      await expect(authenticatedPage).toHaveURL(/.*\/sales\/proposal\//);
    }
  });
});
