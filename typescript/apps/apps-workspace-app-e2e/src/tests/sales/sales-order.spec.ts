...
import { SalesOrderListPage } from '../../pages/sales/sales-order-list.page';

test.describe('Sales Order List', () => {
  test('should display sales order list page with table', async ({
    authenticatedPage,
  }) => {
    const listPage = new SalesOrderListPage(authenticatedPage);
    await listPage.goto();
  ...
  test('should navigate to overview on row click', async ({
    ...
    if (rowCount > 0) {
      ...
      await expect(authenticatedPage).toHaveURL(/.*\/sales\/salesorder\//);
    }
  });
});
