...
import { PurchaseOrderListPage } from '../../pages/purchasing/purchase-order-list.page';

test.describe('Purchase Order List', () => {
  test('should display purchase order list page with table', async ({
    authenticatedPage,
  }) => {
    const listPage = new PurchaseOrderListPage(authenticatedPage);
    await listPage.goto();
  ...
  test('should navigate to overview on row click', async ({
    ...
    if (rowCount > 0) {
      ...
      await expect(authenticatedPage).toHaveURL(/.*\/purchasing\/purchaseorder\//);
    }
  });
});
