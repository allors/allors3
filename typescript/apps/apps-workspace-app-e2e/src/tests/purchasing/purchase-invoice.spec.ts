...
import { PurchaseInvoiceListPage } from '../../pages/purchasing/purchase-invoice-list.page';

test.describe('Purchase Invoice List', () => {
  test('should display purchase invoice list page with table', async ({
    authenticatedPage,
  }) => {
    const listPage = new PurchaseInvoiceListPage(authenticatedPage);
    await listPage.goto();
  ...
  test('should navigate to overview on row click', async ({
    ...
    if (rowCount > 0) {
      ...
      await expect(authenticatedPage).toHaveURL(/.*\/purchasing\/purchaseinvoice\//);
    }
  });
});
