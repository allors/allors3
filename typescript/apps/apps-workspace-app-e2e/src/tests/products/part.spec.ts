...
import { PartListPage } from '../../pages/products/part-list.page';

test.describe('Part List', () => {
  test('should display part list page with table', async ({
    authenticatedPage,
  }) => {
    const listPage = new PartListPage(authenticatedPage);
    await listPage.goto();
  ...
  test('should navigate to overview on row click', async ({
    ...
    if (rowCount > 0) {
      ...
      await expect(authenticatedPage).toHaveURL(/.*\/products\/nonunifiedpart\//);
    }
  });
});
