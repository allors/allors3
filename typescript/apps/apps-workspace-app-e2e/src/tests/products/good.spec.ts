...
import { GoodListPage } from '../../pages/products/good-list.page';

test.describe('Good List', () => {
  test('should display good list page with table', async ({
    authenticatedPage,
  }) => {
    const listPage = new GoodListPage(authenticatedPage);
    await listPage.goto();
  ...
  test('should navigate to overview on row click', async ({
    ...
    if (rowCount > 0) {
      ...
      await expect(authenticatedPage).toHaveURL(/.*\/products\/nonunifiedgood\//);
    }
  });
});
