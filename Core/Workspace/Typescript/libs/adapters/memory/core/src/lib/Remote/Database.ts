    ...
    const syncRequest: SyncRequest = {
      objects: (response.objects ?? [])
        .filter((syncRequestObject) => {
          ...
          const sync = databaseObject == null || databaseObject.version !== version || databaseObject.sortedAccessControlIds !== sortedAccessControlIds || databaseObject.sortedDeniedPermissionIds !== sortedDeniedPermissionIds;
  ...
  sync(syncResponse: SyncResponse): SecurityRequest | undefined {
    ...
    if (syncResponse.objects) {
      syncResponse.objects.forEach((v) => {
        const databaseObject = new MemoryRecord(this, this.metaPopulation, v, sortedAccessControlIdsDecompress, sortedDeniedPermissionIdsDecompress);
