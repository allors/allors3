...
namespace Allors.Workspace.Adapters.Direct
{
    ...
    public class Session : ISession
    {
        ...
        {
            var pullRequest = new PullRequest { Pulls = pulls };
            var pullResponse = this.WorkspaceDatabase.Load(pullRequest);
            ...
            if (pullResponse.NamedCollections != null)
            {
                ...
                {
                    var name = kvp.Key;
                    var collection = this.Instantiate(kvp.Value.Select(v => v.ObjectId)).ToArray();
                    loadResult.Collections.Add(name, collection);
                }
            }
            ...
            if (pullResponse.NamedObjects != null)
            {
                ...
                {
                    var name = kvp.Key;
                    var @object = this.Instantiate(kvp.Value.ObjectId);
                    loadResult.Objects.Add(name, @object);
                }
            }

            if (pullResponse.NamedValues != null)
            {
                foreach (var kvp in pullResponse.NamedValues)
                {
                    var name = kvp.Key;
                    var value = kvp.Key;
                    loadResult.Values.Add(name, value);
                }
            }
