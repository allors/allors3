...
namespace Allors.Workspace.Adapters.Direct
{
    ...
    using Meta;
    using Polly;
    using Protocol.Direct.Api;
    using Protocol.Direct.Api.Pull;
    using AccessControl = Security.AccessControl;
    using Permission = Security.Permission;
    ...
    public class WorkspaceDatabase
    {
        ...
        public WorkspaceDatabase(IMetaPopulation metaPopulation, Database.IDatabase database)
        {
        ...
        public Database.IDatabase Database { get; }
        ...
        internal PullResponse Load(PullRequest pullRequest)
        {
            using var session = this.Database.CreateSession();
            var api = new Api(session, "Default");
            var pullResponse = api.Pull(pullRequest);

            if (pullResponse.NamedCollections != null)
            {
                foreach (var kvp in pullResponse.NamedCollections)
                {
                    foreach (var strategy in kvp.Value)
                    {
                        this.Import(strategy);
                    }
                }
            }

            if (pullResponse.NamedObjects != null)
            {
                foreach (var kvp in pullResponse.NamedObjects)
                {
                    this.Import(kvp.Value);
                }
            }
        ...
        private void Import(Database.IStrategy strategy)
        {
            var databaseId = strategy.ObjectId;
            if (!this.databaseObjectByDatabaseId.TryGetValue(databaseId, out var databaseObject))
            {
                var @class = (IClass)this.MetaPopulation.Find(strategy.Class.Id);
                databaseObject = new DatabaseObject(this, databaseId, @class);
                this.databaseObjectByDatabaseId[databaseId] = databaseObject;
            }
        }
    }
