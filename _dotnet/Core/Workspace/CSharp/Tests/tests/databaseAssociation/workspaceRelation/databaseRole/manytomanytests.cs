                ...
                foreach (DatabaseMode mode1 in Enum.GetValues(typeof(DatabaseMode)))
                {
                    foreach (DatabaseMode mode2 in Enum.GetValues(typeof(DatabaseMode)))
                    {
                        foreach (var contextFactory in this.contextFactories)
                        {
                            ...
                            Assert.Single(c1x_1.WorkspaceC1Many2Manies);
                            Assert.Single(c1y_1.C1sWhereWorkspaceC1Many2Many);
                            c1x_1.WorkspaceC1Many2Manies.ShouldContain(c1y_1, ctx, mode1, mode2);
                            c1y_1.C1sWhereWorkspaceC1Many2Many.ShouldContain(c1x_1, ctx, mode1, mode2);

                            await push(session1);

                            Assert.Single(c1x_1.WorkspaceC1Many2Manies);
                            Assert.Single(c1y_1.C1sWhereWorkspaceC1Many2Many);
                            c1x_1.WorkspaceC1Many2Manies.ShouldContain(c1y_1, ctx, mode1);
                            c1y_1.C1sWhereWorkspaceC1Many2Many.ShouldContain(c1x_1, ctx, mode1, mode2);
                        }
                    }
                }
                ...
                foreach (DatabaseMode mode1 in Enum.GetValues(typeof(DatabaseMode)))
                {
                    foreach (DatabaseMode mode2 in Enum.GetValues(typeof(DatabaseMode)))
                    {
                        foreach (var contextFactory in this.contextFactories)
                        {
                            ...
                            Assert.Single(c1y_1.C1sWhereWorkspaceC1Many2Many.Where(v => v.Equals(c1x_1)));
                ...
                foreach (DatabaseMode mode1 in Enum.GetValues(typeof(DatabaseMode)))
                {
                    foreach (DatabaseMode mode2 in Enum.GetValues(typeof(DatabaseMode)))
                    {
                        foreach (var contextFactory in this.contextFactories)
                        {
                            ...
                            c1y_1.C1sWhereWorkspaceC1Many2Many.ShouldNotContain(c1x_1, ctx, mode1, mode2);
