    ...
    public class PurchaseOrderPriceDerivation : DomainDerivation
    {
        public PurchaseOrderPriceDerivation(M m) : base(m, new Guid("b553564c-45e1-495c-975a-90eb6fc67d5d")) =>
            this.Patterns = new Pattern[]
            {
                ...
                new ChangedPattern(this.M.DiscountAdjustment.Percentage) { Steps =  new IPropertyType[] {m.DiscountAdjustment.PriceableWhereDiscountAdjustment, m.PurchaseOrderItem.PurchaseOrderWherePurchaseOrderItem}, OfType = m.PurchaseOrder.Class },
                new ChangedPattern(this.M.DiscountAdjustment.Percentage) { Steps =  new IPropertyType[] {m.OrderAdjustment.OrderWhereOrderAdjustment}, OfType = m.PurchaseOrder.Class },
                new ChangedPattern(this.M.DiscountAdjustment.Amount) { Steps =  new IPropertyType[] {m.DiscountAdjustment.PriceableWhereDiscountAdjustment, m.PurchaseOrderItem.PurchaseOrderWherePurchaseOrderItem}, OfType = m.PurchaseOrder.Class },
                new ChangedPattern(this.M.DiscountAdjustment.Amount) { Steps =  new IPropertyType[] {m.OrderAdjustment.OrderWhereOrderAdjustment}, OfType = m.PurchaseOrder.Class },
                new ChangedPattern(this.M.SurchargeAdjustment.Percentage) { Steps =  new IPropertyType[] {m.SurchargeAdjustment.PriceableWhereSurchargeAdjustment, m.PurchaseOrderItem.PurchaseOrderWherePurchaseOrderItem}, OfType = m.PurchaseOrder.Class },
                new ChangedPattern(this.M.SurchargeAdjustment.Percentage) { Steps =  new IPropertyType[] {m.OrderAdjustment.OrderWhereOrderAdjustment}, OfType = m.PurchaseOrder.Class },
                new ChangedPattern(this.M.SurchargeAdjustment.Amount) { Steps =  new IPropertyType[] {m.SurchargeAdjustment.PriceableWhereSurchargeAdjustment, m.PurchaseOrderItem.PurchaseOrderWherePurchaseOrderItem}, OfType = m.PurchaseOrder.Class },
                new ChangedPattern(this.M.SurchargeAdjustment.Amount) { Steps =  new IPropertyType[] {m.OrderAdjustment.OrderWhereOrderAdjustment}, OfType = m.PurchaseOrder.Class },
            };
            ...
            foreach (var @this in matches.Cast<PurchaseOrder>())
            {
                foreach (PurchaseOrderItem purchaseOrderItem in @this.ValidOrderItems)
                {
                    ...
                    else
                    {
                        ...
                        if (order != null)
                        {
                            purchaseOrderItem.UnitBasePrice = new SupplierOfferings(@this.Strategy.Session).PurchasePrice(order.TakenViaSupplier, order.OrderDate, purchaseOrderItem.Part);
                        }
                    }
